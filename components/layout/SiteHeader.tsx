...
export function SiteHeader({ variant = "overlay" }: { variant?: "overlay" | "solid" }) {
  ...
  return (
    <header ref={headerRef} onBlur={(event) => { if (open && !event.currentTarget.contains(event.relatedTarget)) setOpen(false); }} data-scrolled={scrolled} data-menu-open={open} className={`fixed inset-x-0 top-0 z-50 border-b transition-[background-color,backdrop-filter,border-color] duration-200 motion-reduce:transition-none ${open ? "max-h-dvh overflow-y-auto border-transparent bg-surface text-brand" : solid ? "border-border bg-surface text-brand" : scrolled ? "border-[#A6AABF]/30 bg-white/[0.03] text-white backdrop-blur-[8px]" : "border-transparent bg-transparent text-white"}`}>
      ...
      <div className="mx-auto flex h-[67px] max-w-site items-center justify-between px-8 md:h-[101px] md:px-0 min-[768px]:max-[1137px]:mx-8">
        <a href={`${homeHrefPrefix}#hero`} aria-label="مقود — الرئيسية" onClick={() => setOpen(false)}>
          <Image src={open || solid ? "/brand/miqwad-blue.svg" : "/brand/miqwad-white.svg"} alt="مقود" width={100} height={36} className="h-9 w-[100px]" />
        </a>
        <nav aria-label="التنقل الرئيسي" className="hidden md:block">
          <ul className="flex items-center gap-8 text-base font-medium leading-6">
            <li><a href={`${homeHrefPrefix}#hero`} onClick={handleNavClick} aria-current={solid ? undefined : "page"}>الرئيسية</a></li>
            <li><a href={`${homeHrefPrefix}#services`} onClick={handleNavClick}>الخدمات</a></li>
            <li><a href={`${homeHrefPrefix}#how-it-works`} onClick={handleNavClick}>آلية العمل</a></li>
          </ul>
        </nav>
        <a href={`${homeHrefPrefix}#download`} className="hidden h-[35px] w-32 items-center justify-center rounded-control bg-accent text-sm font-medium text-white md:flex">تحميل التطبيق</a>
      ...
      {open ? <MobileNavigation homeHrefPrefix={homeHrefPrefix} onClose={() => setOpen(false)} onNavClick={solid ? undefined : handleNavClick} /> : <div id="mobile-navigation" hidden />}
    </header>
  );
}
