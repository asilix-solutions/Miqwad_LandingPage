...
export function SiteHeader() {
  ...
  return (
    <header ref={headerRef} onBlur={(event) => { if (open && !event.currentTarget.contains(event.relatedTarget)) setOpen(false); }} data-scrolled={scrolled} data-menu-open={open} className={`fixed inset-x-0 top-0 z-50 border-b transition-[background-color,backdrop-filter,border-color] duration-200 motion-reduce:transition-none ${open ? "max-h-dvh overflow-y-auto border-transparent bg-surface text-brand" : scrolled ? "border-white/10 bg-brand/75 text-white backdrop-blur-[8px]" : "border-transparent bg-transparent text-white"}`}>
